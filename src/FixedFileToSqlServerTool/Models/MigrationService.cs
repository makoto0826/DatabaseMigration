...
public class MigrationService
{
    ...
    public MigrationService(SqlConnection connection, ScriptRunner scriptRunner)
    {
        ...
        while ((line = reader.ReadLine()) != null)
        {
            foreach (var column in context.Table.Columns)
            {
                ...
                if (column.IsGeneration)
                {
                    ...
                    {
                        throw new ServiceException("生成スクリプトが設定されていません");
                    }
                ...
                else
                {
                    ...
                    {
                        throw new ServiceException($"列名:{column.Destination.Name} 開始位置または終了位置が設定されていません");
                    }
